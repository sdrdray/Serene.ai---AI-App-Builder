 ...
 */
export default function sereneTagger(): Plugin {
  return {
    name: "vite-plugin-serene-tagger",
    apply: "serve",
    enforce: "pre",
    ...
    async transform(code: string, id: string) {
      try {
        ...
        walk(ast as any, {
          enter(node: any) {
            try {
              ...
              // ── 2. Check whether the tag already has data-serene-id ───────────────
              ...
              console.warn(
                `[serene-tagger] Warning: Failed to process JSX node in ${id}:`,
                error,
              );
            }
          },
        ...
        console.warn(
          `[serene-tagger] Warning: Failed to transform ${id}:`,
          error,
        );
        return null;
      }
