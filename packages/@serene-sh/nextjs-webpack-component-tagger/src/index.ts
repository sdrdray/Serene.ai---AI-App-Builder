...
/**
 * A webpack loader that adds `data-serene-*` attributes to JSX elements.
 */
export default function sereneTaggerLoader(this: any, code: string) {
  ...
  const transform = async () => {
    try {
      ...
      walk(ast as any, {
        enter: (node: any) => {
          try {
            ...
            if (!loc) return;
            const sereneId = `${fileRelative}:${loc.line}:${loc.column}`;

            // Inject the attributes
            if (node.name.end != null) {
              ms.appendLeft(
                node.name.end,
                ` data-serene-id="${sereneId}" data-serene-name="${tagName}"`,
              );
              transformCount++;
            }
          } catch (error) {
            console.warn(
              `[serene-tagger] Warning: Failed to process JSX node in ${this.resourcePath}:`,
              error,
            );
          }
        },
      ...
      console.warn(
        `[serene-tagger] Warning: Failed to transform ${this.resourcePath}:`,
        error,
      );
      return null;
    }
  ...
  transform()
    ...
    .catch((err) => {
      console.error(`[serene-tagger] ERROR in ${this.resourcePath}:`, err);
