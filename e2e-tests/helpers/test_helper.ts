...
export class PageObject {
  ...
  async setUpSerenePro({ autoApprove = false }: { autoApprove?: boolean } = {}) {
    await this.baseSetup();
    await this.goToSettingsTab();
    if (autoApprove) {
      await this.toggleAutoApprove();
    }
    await this.setUpSereneProvider();
    await this.goToAppsTab();
  }
  ...
  async setUpSereneProvider() {
    await this.page
      .locator("div")
      .filter({ hasText: /^SereneNeeds Setup$/ })
      .nth(1)
      .click();
    await this.page.getByRole("textbox", { name: "Set Serene API Key" }).click();
    await this.page
      .getByRole("textbox", { name: "Set Serene API Key" })
      .fill("testserenekey");
  ...
  async snapshotMessages({
    ...
    if (replaceDumpPath) {
      // Update page so that "[[serene-dump-path=*]]" is replaced with a placeholder path
  ...
  async snapshotServerDump(
    ...
    const dumpPaths = dumpPathMatches
      .map((match) => {
        const pathMatch = match.match(/\[\[serene-dump-path=([^\]]+)\]\]/);
    ...
    ).replaceAll(/\[\[serene-dump-path=([^\]]+)\]\]/g, "[[serene-dump-path=*]]");
  ...
  getChatInput() {
    return this.page.getByRole("textbox", { name: "Ask Serene to build..." });
  }
  ...
  getAppPath({ appName }: { appName: string }) {
    return path.join(this.userDataDir, "serene-apps", appName);
  }
...
export const test = base.extend<{
  ...
  po: [
    async ({ electronApp }, use) => {
      ...
      const po = new PageObject(electronApp, page, {
        userDataDir: (electronApp as any).$sereneUserDataDir,
      });
      await use(po);
    },
    { auto: true },
  ...
  electronApp: [
    async ({ electronConfig }, use) => {
      ...
      process.env.SERENE_ENGINE_URL = "http://localhost:3500/engine/v1";
      process.env.SERENE_GATEWAY_URL = "http://localhost:3500/gateway/v1";
      process.env.E2E_TEST_BUILD = "true";
      // This is just a hack to avoid the AI setup screen.
      process.env.OPENAI_API_KEY = "sk-test";
      const baseTmpDir = os.tmpdir();
      const userDataDir = path.join(baseTmpDir, `serene-e2e-tests-${Date.now()}`);
      ...
      (electronApp as any).$sereneUserDataDir = userDataDir;
      ...
      if (os.platform() === "win32") {
        try {
          console.log("[cleanup:start] Killing serene.exe");
          console.time("taskkill");
          execSync("taskkill /f /t /im serene.exe");
          console.timeEnd("taskkill");
          console.log("[cleanup:end] Killed serene.exe");
        } catch (error) {
          console.warn(
            "Failed to kill serene.exe: (continuing with test cleanup)",
            error,
          );
        }
      } else {
    ...
    .map((message) => {
      const content = Array.isArray(message.content)
        ...
        : message.content
            ...
            .replace(
              /\n<serene-file path="package\.json">[\s\S]*?<\/serene-file>\n/g,
              "",
            );
      return `===\nrole: ${message.role}\nmessage: ${content}`;
    })
