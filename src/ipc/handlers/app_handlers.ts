...
import { getSereneAppPath, getUserDataPath } from "../../paths/paths";
    ...
    if (urlMatch) {
      proxyWorker = await startProxy(urlMatch[1], {
        onStarted: (proxyUrl) => {
          safeSend(event.sender, "app:output", {
            type: "stdout",
            message: `[serene-proxy-server]started=[${proxyUrl}] original=[${urlMatch[1]}]`,
            appId,
          });
        },
      });
...
export function registerAppHandlers() {
  ...
  handle(
    ...
    async (
      ...
      const fullAppPath = getSereneAppPath(appPath);
  ...
  handle(
    ...
    async (_, params: CopyAppParams): Promise<{ app: any }> => {
      ...
      const originalAppPath = getSereneAppPath(originalApp.path);
      const newAppPath = getSereneAppPath(newAppName);
  ...
  handle("get-app", async (_, appId: number): Promise<App> => {
    ...
    // Get app files
    const appPath = getSereneAppPath(app.path);
    let files: string[] = [];
  ...
  ipcMain.handle("list-apps", async () => {
    ...
    return {
      apps: allApps,
      appBasePath: getSereneAppPath("$APP_BASE_PATH"),
    };
  });
  ...
  ipcMain.handle(
    ...
    async (
      ...
      return withLock(appId, async () => {
        ...
        const oldAppPath = getSereneAppPath(app.path);
        const newAppPath = getSereneAppPath(appPath);
  ...
  ipcMain.handle("reset-all", async (): Promise<void> => {
    ...
    const sereneAppPath = getSereneAppPath(".");
    if (fs.existsSync(sereneAppPath)) {
      await fsPromises.rm(sereneAppPath, { recursive: true, force: true });
      // Recreate the base directory
      await fsPromises.mkdir(sereneAppPath, { recursive: true });
    }
