...
const TEST_RESPONSES: Record<string, string> = {
  ...
  <serene-add-dependency packages="deno"></serene-add-dependency>
  
  EOM`,
  "add-non-existing-dep": `I'll add that dependency for you.
  
  <serene-add-dependency packages="@angular/does-not-exist"></serene-add-dependency>
  
  EOM`,
  "add-multiple-deps": `I'll add that dependency for you.
  
  <serene-add-dependency packages="react-router-dom react-query"></serene-add-dependency>
  
  EOM`,
  write: `Hello world
  <serene-write path="src/hello.ts" content="Hello world">
...
export function getTestResponse(prompt: string): string | null {
  const match = prompt.match(/\[serene-qa=([^\]]+)\]/);
  if (match) {
