...
import { getSereneAppPath } from "../../paths/paths";
...
const TEMP_DIR = path.join(os.tmpdir(), "serene-attachments");
...
async function processStreamChunks({
  ...
  for await (const part of fullStream) {
    ...
    if (part.type === "text-delta") {
      ...
      chunk += escapeSereneTags(part.textDelta);
    }
      ...
      if (req.attachments && req.attachments.length > 0) {
        ...
        for (const [index, attachment] of req.attachments.entries()) {
          ...
          if (attachment.attachmentType === "upload-to-codebase") {
            ...
            const fileId = `SERENE_ATTACHMENT_${index}`;
            ...
            // Add instruction for AI to use serene-write tag
            ...
            if (await isTextFile(filePath)) {
              try {
                attachmentInfo += `<serene-text-attachment filename="${attachment.name}" type="${attachment.type}" path="${filePath}">
                </serene-text-attachment>
                \n\n`;
              } catch (err) {
      ...
      if (req.selectedComponent) {
        ...
        try {
          const componentFileContent = await readFile(
            path.join(
              getSereneAppPath(chat.app.path),
...
`;
      ...
      if (testResponse) {
        ...
        const appPath = getSereneAppPath(updatedChat.app.path);
        ...
        let systemPrompt = constructSystemPrompt({
          aiRules: await readAiRules(getSereneAppPath(updatedChat.app.path)),
        ...
        // print out the serene-write tags.
...
<serene-write path="path/to/destination/filename.ext" description="Upload file to codebase">
SERENE_ATTACHMENT_X
</serene-write>

Example for file with id of SERENE_ATTACHMENT_0:
<serene-write path="src/components/Button.jsx" description="Upload file to codebase">
SERENE_ATTACHMENT_0
</serene-write>
...
`;
          ...
          ...limitedMessageHistory.map((msg) => ({
            ...
            content:
              settings.selectedChatMode === "ask"
                ? removeSereneTags(removeNonEssentialTags(msg.content))
        ...
        const simpleStreamText = async ({
          ...
          return streamText({
            ...
            providerOptions: {
              ...
              "serene-gateway": getExtraProviderOptions(
        ...
        try {
          ...
          if (
            ...
            settings.selectedChatMode !== "ask" &&
            hasUnclosedSereneWrite(fullResponse)
          ) {
            ...
            while (
              ...
              logger.warn(
                `Received unclosed serene-write tag, attempting to continue, attempt #${continuationAttempts + 1}`,
              );
          ...
          const addDependencies = getSereneAddDependencyTags(fullResponse);
          if (
            !abortController.signal.aborted &&
            ...
            try {
              ...
              let problemReport = await generateProblemReport({
                fullResponse,
                appPath: getSereneAppPath(updatedChat.app.path),
              });
              ...
              while (
                ...
                fullResponse += `<serene-problem-report summary="${problemReport.problems.length} problems">
${problemReport.problems
  ...
  .join("\n")}
                ...
                const virtualFileSystem = new AsyncVirtualFileSystem(
                  getSereneAppPath(updatedChat.app.path),
                  {
                    fileExists: (fileName: string) => fileExists(fileName),
                    readFile: (fileName: string) => readFileWithCache(fileName),
                  },
                );
                const writeTags = getSereneWriteTags(fullResponse);
                const renameTags = getSereneRenameTags(fullResponse);
                const deletePaths = getSereneDeleteTags(fullResponse);
...
[Response cancelled by user]`,
      ...
      if (!abortController.signal.aborted && fullResponse) {
        // Scrape from: <serene-chat-summary>Renaming profile file</serene-chat-title>
        const chatTitle = fullResponse.match(
          /<serene-chat-summary>(.*?)<\/serene-chat-summary>/,
        );
        if (chatTitle) {
          await db
            .update(chats)
  ...
  try {
    if (await isTextFile(filePath)) {
      ...
      const tagPattern = new RegExp(
        `<serene-text-attachment filename="[^"]*" type="[^"]*" path="${escapedPath}">\\s*<\\/serene-text-attachment>`,
        "g",
      );
...
export function removeProblemReportTags(text: string): string {
  const problemReportRegex =
    /<serene-problem-report[^>]*>[\s\S]*?<\/serene-problem-report>/g;
  return text.replace(problemReportRegex, "").trim();
}

export function removeSereneTags(text: string): string {
  const sereneRegex = /<serene-[^>]*>[\s\S]*?<\/serene-[^>]*>/g;
  return text.replace(sereneRegex, "").trim();
}

export function hasUnclosedSereneWrite(text: string): boolean {
  // Find the last opening serene-write tag
  const openRegex = /<serene-write[^>]*>/g;
  let lastOpenIndex = -1;
  let match;
  ...
  const hasClosingTag = /<\/serene-write>/.test(textAfterLastOpen);

  return !hasClosingTag;
}

function escapeSereneTags(text: string): string {
  // Escape serene tags in reasoning content
  // We are replacing the opening tag with a look-alike character
  // to avoid issues where thinking content includes serene tags
  // and are mishandled by:
  // 1. FE markdown parser
  // 2. Main process response processor
  return text.replace(/<serene/g, "＜serene").replace(/<\/serene/g, "＜/serene");
}
