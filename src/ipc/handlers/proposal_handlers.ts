...
import {
  ...
  getSereneChatSummaryTag,
  getSereneCommandTags,
...
async function getCodebaseTokenCount(
  ...
  const codebase = (
    await extractCodebase({
      appPath: getSereneAppPath(appPath),
  ...
  return withLock("get-proposal:" + chatId, async () => {
    ...
    try {
      ...
      if (
        ...
        const proposalTitle = getSereneChatSummaryTag(messageContent);

        const proposalWriteFiles = getSereneWriteTags(messageContent);
        const proposalRenameFiles = getSereneRenameTags(messageContent);
        const proposalDeleteFiles = getSereneDeleteTags(messageContent);
        const proposalExecuteSqlQueries = getSereneExecuteSqlTags(messageContent);
        const packagesAdded = getSereneAddDependencyTags(messageContent);
      ...
      if (latestAssistantMessage?.content) {
        const writeTags = getSereneWriteTags(latestAssistantMessage.content);
        ...
        const commandTags = getSereneCommandTags(latestAssistantMessage.content);
...
const approveProposalHandler = async (
  ...
  const chatSummary = getSereneChatSummaryTag(messageToApprove.content);
