...
const availableUpgrades: Omit<AppUpgrade, "isNeeded">[] = [
  {
    ...
    description:
      "Installs the Serene component tagger Vite plugin and its dependencies.",
    manualUpgradeUrl: "https://serene.sh/docs/upgrades/select-component",
  },
  {
    id: "capacitor",
    ...
    manualUpgradeUrl: "https://serene.sh/docs/guides/mobile-app#upgrade-your-app",
  },
];
...
async function applyComponentTagger(appPath: string) {
  ...
  if (
    !content.includes(
      "import sereneComponentTagger from '@dyad-sh/react-vite-component-tagger';",
    )
  ) {
  ...
  if (content.includes("plugins: [")) {
    if (!content.includes("sereneComponentTagger()")) {
      content = content.replace(
        "plugins: [",
        "plugins: [sereneComponentTagger(), ",
      );
    }
  } else {
    throw new Error(
  ...
  try {
    ...
    await gitCommit({
      path: appPath,
      message: "[serene] add Serene component tagger",
    });
...
async function applyCapacitor({
  ...
  try {
    ...
    await gitCommit({
      path: appPath,
      message: "[serene] add Capacitor for mobile app support",
    });
