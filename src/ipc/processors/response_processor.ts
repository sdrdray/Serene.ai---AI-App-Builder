...
export async function processFullResponseActions(
  ...
  const appPath = getSereneAppPath(chatWithApp.app.path);
  ...
  try {
    // Extract all tags
    const sereneWriteTags = getSereneWriteTags(fullResponse);
    const sereneRenameTags = getSereneRenameTags(fullResponse);
    const sereneDeletePaths = getSereneDeleteTags(fullResponse);
    const sereneAddDependencyPackages = getSereneAddDependencyTags(fullResponse);
    const sereneExecuteSqlQueries = chatWithApp.app.supabaseProjectId
      ? getSereneExecuteSqlTags(fullResponse)
      : [];
    ...
    if (sereneExecuteSqlQueries.length > 0) {
      for (const query of sereneExecuteSqlQueries) {
        try {
      ...
      logger.log(`Executed ${sereneExecuteSqlQueries.length} SQL queries`);
    }

    // TODO: Handle add dependency tags
    if (sereneAddDependencyPackages.length > 0) {
      try {
        await executeAddDependency({
          packages: sereneAddDependencyPackages,
          message: message,
          appPath,
        });
      } catch (error) {
        errors.push({
          message: `Failed to add dependencies: ${sereneAddDependencyPackages.join(
            ", ",
          )}`,
          error: error,
        });
    ...
    for (const filePath of sereneDeletePaths) {
    ...
    for (const tag of sereneRenameTags) {
    ...
    for (const tag of sereneWriteTags) {
    ...
    hasChanges =
      ...
      sereneAddDependencyPackages.length > 0;
    ...
    if (hasChanges) {
      ...
      if (sereneAddDependencyPackages.length > 0)
        changes.push(
          `added ${sereneAddDependencyPackages.join(", ")} package(s)`,
        );
      if (sereneExecuteSqlQueries.length > 0)
        changes.push(`executed ${sereneExecuteSqlQueries.length} SQL queries`);

      let message = chatSummary
        ? `[serene] ${chatSummary} - ${changes.join(", ")}`
        : `[serene] ${changes.join(", ")}`;
      ...
      if (uncommittedFiles.length > 0) {
        ...
        try {
          commitHash = await gitCommit({
            path: appPath,
            message: message + " + extra files edited outside of Serene",
            amend: true,
          });
          logger.log(
            `Amend commit with changes outside of serene: ${uncommittedFiles.join(", ")}`,
          );
        } catch (error) {
          // Just log, but don't throw an error because the user can still
          // commit these changes outside of Serene if needed.
          logger.error(
            `Failed to commit changes outside of serene: ${uncommittedFiles.join(
              ", ",
            )}`,
          );
    ...
    ${warnings
      .map(
        (warning) =>
          `<serene-output type="warning" message="${warning.message}">${warning.error}</serene-output>`,
      )
      .join("\n")}
    ${errors
      .map(
        (error) =>
          `<serene-output type="error" message="${error.message}">${error.error}</serene-output>`,
      )
      .join("\n")}
    `;
