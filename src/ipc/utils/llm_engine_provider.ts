...
export interface SereneEngineProvider {
  /**
...
export function createSereneEngine(
  options: ExampleProviderSettings,
): SereneEngineProvider {
  const baseURL = withoutTrailingSlash(options.baseURL);
  logger.info("creating serene engine with baseURL", baseURL);
  ...
  const getHeaders = () => ({
    Authorization: `Bearer ${loadApiKey({
      ...
      environmentVariableName: "SERENE_PRO_API_KEY",
    ...
    const config = {
      ...
      fetch: (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        try {
          ...
          const requestId = parsedBody.sereneRequestId;
          if ("sereneRequestId" in parsedBody) {
            delete parsedBody.sereneRequestId;
          }
          ...
          if (files?.length) {
            parsedBody.serene_options = {
              files,
              enable_lazy_edits: options.sereneOptions.enableLazyEdits,
              enable_smart_files_context:
                options.sereneOptions.enableSmartFilesContext,
            };
          }
          ...
          const modifiedInit = {
            ...
            headers: {
              ...init.headers,
              ...(modifiedRequestId && {
                "X-Serene-Request-Id": modifiedRequestId,
              }),
            },
