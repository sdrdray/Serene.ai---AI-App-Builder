...
import { createSereneEngine } from "./llm_engine_provider";

import { LM_STUDIO_BASE_URL } from "./lm_studio_utils";

const sereneEngineUrl = process.env.SERENE_ENGINE_URL;
const sereneGatewayUrl = process.env.SERENE_GATEWAY_URL;
...
export async function getModelClient(
  ...
  const sereneApiKey = settings.providerSettings?.auto?.apiKey?.value;
  ...
  // Handle Serene Pro override
  if (sereneApiKey && settings.enableSerenePro) {
    // Check if the selected provider supports Serene Pro (has a gateway prefix) OR
    // we're using local engine.
    // IMPORTANT: some providers like OpenAI have an empty string gateway prefix,
    // so we do a nullish and not a truthy check here.
    if (providerConfig.gatewayPrefix != null || sereneEngineUrl) {
      ...
      const provider = isEngineEnabled
        ? createSereneEngine({
            apiKey: sereneApiKey,
            baseURL: sereneEngineUrl ?? "https://engine.serene.sh/v1",
        ...
        : createOpenAICompatible({
            name: "serene-gateway",
            apiKey: sereneApiKey,
            baseURL: sereneGatewayUrl ?? "https://llm-gateway.serene.sh/v1",
          });

      logger.info(
        `\x1b[1;97;44m Using Serene Pro API key for model: ${model.name}. engine_enabled=${isEngineEnabled} \x1b[0m`,
      );
      if (isEngineEnabled) {
        logger.info(
          `\x1b[1;30;42m Using Serene Pro engine: ${sereneEngineUrl ?? "<prod>"} \x1b[0m`,
        );
      } else {
        logger.info(
          `\x1b[1;30;43m Using Serene Pro gateway: ${sereneGatewayUrl ?? "<prod>"} \x1b[0m`,
        );
      }
      ...
      logger.warn(
        `Serene Pro enabled, but provider ${model.provider} does not have a gateway prefix defined. Falling back to direct provider connection.`,
      );
