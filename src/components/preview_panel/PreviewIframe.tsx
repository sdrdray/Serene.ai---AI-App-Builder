...
export const PreviewIframe = ({ loading }: { loading: boolean }) => {
    ...
    if (!selectedComponentPreview) {
      if (iframeRef.current?.contentWindow) {
        iframeRef.current.contentWindow.postMessage(
          { type: "deactivate-serene-component-selector" },
          "*",
        );
      }
      setIsPicking(false);
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data?.type === "serene-component-selector-initialized") {
        setIsComponentSelectorInitialized(true);
        return;
      }

      if (event.data?.type === "serene-component-selected") {
  ...
  const handleActivateComponentSelector = () => {
    if (iframeRef.current?.contentWindow) {
      ...
      iframeRef.current.contentWindow.postMessage(
        {
          type: newIsPicking
            ? "activate-serene-component-selector"
            : "deactivate-serene-component-selector",
        },
        "*",
      );
    }
...
function parseComponentSelection(data: any): ComponentSelection | null {
  if (
    !data ||
    data.type !== "serene-component-selected" ||
