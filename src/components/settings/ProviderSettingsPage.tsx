...
export function ProviderSettingsPage({ provider }: ProviderSettingsPageProps) {
  ...
  // Use fetched data (or defaults for Serene)
  const providerDisplayName = isSerene
    ? "Serene"
    : (providerData?.name ?? "Unknown Provider");
  const providerWebsiteUrl = isSerene
    ? "https://academy.serene.sh/settings"
    : providerData?.websiteUrl;
  const hasFreeTier = isSerene ? false : providerData?.hasFreeTier;
  const envVarName = isSerene ? undefined : providerData?.envVarName;
  ...
  const handleSaveKey = async () => {
    ...
    try {
      ...
      if (isSerene) {
        settingsUpdate.enableSerenePro = true;
      }
  ...
  // --- Toggle Serene Pro Handler ---
  const handleToggleSerenePro = async (enabled: boolean) => {
    setIsSaving(true);
    try {
      await updateSettings({
        enableSerenePro: enabled,
      });
    } catch (error: any) {
      showError(`Error toggling Serene Pro: ${error}`);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="min-h-screen px-8 py-4">
      <div className="max-w-4xl mx-auto">
        ...
        {isSerene && !settingsLoading && (
          <div className="mt-6 flex items-center justify-between p-4 bg-(--background-lightest) rounded-lg border">
            <div>
              <h3 className="font-medium">Enable Serene Pro</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Toggle to enable Serene Pro
              </p>
            </div>
            <Switch
              checked={settings?.enableSerenePro}
              onCheckedChange={handleToggleSerenePro}
              disabled={isSaving}
            />
          </div>
        )}
