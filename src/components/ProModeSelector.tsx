...
export function ProModeSelector() {
  ...
  const toggleProEnabled = () => {
    updateSettings({
      enableSerenePro: !settings?.enableSerenePro,
    });
  };

  const hasProKey = settings ? hasSereneProKey(settings) : false;
  const proModeTogglable = hasProKey && Boolean(settings?.enableSerenePro);
  ...
  return (
    <Popover>
      <Tooltip>
        ...
        <TooltipContent>Configure Serene Pro settings</TooltipContent>
      </Tooltip>
        ...
        <div className="space-y-4">
          <div className="space-y-1">
            <h4 className="font-medium flex items-center gap-1.5">
              <Sparkles className="h-4 w-4 text-primary" />
              <span className="text-primary font-medium">Serene Pro</span>
            </h4>
          ...
          <div className="flex flex-col gap-5">
            <SelectorRow
              id="pro-enabled"
              label="Enable Serene Pro"
              description="Use Serene Pro AI credits"
              tooltip="Uses Serene Pro AI credits for the main AI model and Pro modes."
              isTogglable={hasProKey}
              settingEnabled={Boolean(settings?.enableSerenePro)}
