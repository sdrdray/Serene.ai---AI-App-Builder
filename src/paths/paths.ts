...
export function getSereneAppPath(appPath: string): string {
  if (IS_TEST_BUILD) {
    const electron = getElectron();
    return path.join(electron!.app.getPath("userData"), "serene-apps", appPath);
  }
  return path.join(os.homedir(), "serene-apps", appPath);
}
