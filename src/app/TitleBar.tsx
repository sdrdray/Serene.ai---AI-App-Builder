...
import { SereneProSuccessDialog } from "@/components/SereneProSuccessDialog";
...
export const TitleBar = () => {
  ...
  const showSereneProSuccessDialog = () => {
  ...
  useEffect(() => {
    const handleDeepLink = async () => {
      if (lastDeepLink?.type === "serene-pro-return") {
        await refreshSettings();
        showSereneProSuccessDialog();
      }
    };
    handleDeepLink();
  }, [lastDeepLink]);
  ...
  const isSerenePro = !!settings?.providerSettings?.auto?.apiKey?.value;
  const isSereneProEnabled = Boolean(settings?.enableSerenePro);
  ...
  return (
    <>
      <div className="@container z-11 w-full h-11 bg-(--sidebar) absolute top-0 left-0 app-region-drag flex items-center">
        ...
        <img src={logo} alt="Serene Logo" className="w-6 h-6 mr-0.5" />
        <Button
        ...
        {isSerenePro && <SereneProButton isSereneProEnabled={isSereneProEnabled} />}
...
export function SereneProButton({
  isSereneProEnabled,
}: {
  isSereneProEnabled: boolean;
}) {
  const { navigate } = useRouter();
  const { userBudget } = useUserBudgetInfo();
  return (
    <Button
      data-testid="title-bar-serene-pro-button"
      onClick={() => {
        navigate({
      ...
      className={cn(
        ...
        !isSereneProEnabled && "bg-zinc-600 dark:bg-zinc-600",
      )}
      size="sm"
    >
      {isSereneProEnabled ? "Pro" : "Pro (off)"}
      {userBudget && isSereneProEnabled && (
