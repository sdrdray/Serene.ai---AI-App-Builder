...
import { handleSereneProReturn } from "./main/pro";
...
if (process.defaultApp) {
  if (process.argv.length >= 2) {
    app.setAsDefaultProtocolClient("serene", process.execPath, [
      path.resolve(process.argv[1]),
    ]);
  }
} else {
  app.setAsDefaultProtocolClient("serene");
}
...
export async function onReady() {
  ...
  if (settings.enableAutoUpdate) {
    ...
    const host = `https://api.serene.sh/v1/update/${postfix}`;
    logger.info("Auto-update release channel=", postfix);
    updateElectronApp({
      logger,
      updateSource: {
        type: UpdateSourceType.ElectronPublicUpdateService,
        repo: "serene-sh/serene",
        host,
      },
...
function handleDeepLinkReturn(url: string) {
  // example url: "serene://supabase-oauth-return?token=a&refreshToken=b"
  let parsed: URL;
  try {
    parsed = new URL(url);
  } catch {
  ...
  if (parsed.protocol !== "serene:") {
    dialog.showErrorBox(
      "Invalid Protocol",
      `Expected serene://, got ${parsed.protocol}. Full URL: ${url}`,
    );
    return;
  }
  ...
  // serene://serene-pro-return?key=123&budget_reset_at=2025-05-26T16:31:13.492000Z&max_budget=100
  if (parsed.hostname === "serene-pro-return") {
    const apiKey = parsed.searchParams.get("key");
    if (!apiKey) {
      dialog.showErrorBox("Invalid URL", "Expected key");
      return;
    }
    handleSereneProReturn({
      apiKey,
    });
    // Send message to renderer to trigger re-render
    mainWindow?.webContents.send("deep-link-received", {
