...
1. `serene-{version}-1.x86_64.rpm` (Linux RPM)
2. `serene-{version}-full.nupkg` (Windows NuGet package)
3. `serene-{version}.Setup.exe` (Windows installer)
4. `serene-darwin-arm64-{version}.zip` (macOS Apple Silicon)
5. `serene-darwin-x64-{version}.zip` (macOS Intel)
6. `serene_{version}_amd64.deb` (Linux DEB)
