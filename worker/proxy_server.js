...
let sereneShimContent = null;
let sereneComponentSelectorClientContent = null;
try {
...
try {
  const sereneShimPath = path.join(__dirname, "serene-shim.js");
  sereneShimContent = fs.readFileSync(sereneShimPath, "utf-8");
  parentPort?.postMessage("[proxy-worker] serene-shim.js loaded.");
} catch (error) {
  parentPort?.postMessage(
    `[proxy-worker] Failed to read serene-shim.js: ${error.message}`,
  );
}

try {
  const sereneComponentSelectorClientPath = path.join(
    __dirname,
    "serene-component-selector-client.js",
  );
  sereneComponentSelectorClientContent = fs.readFileSync(
    sereneComponentSelectorClientPath,
    "utf-8",
  );
  parentPort?.postMessage(
    "[proxy-worker] serene-component-selector-client.js loaded.",
  );
} catch (error) {
  parentPort?.postMessage(
    `[proxy-worker] Failed to read serene-component-selector-client.js: ${error.message}`,
  );
}
...
function injectHTML(buf) {
  ...
  // These are strings that were used since the first version of the serene shim.
  // If the serene shim is used from legacy apps which came pre-baked with the shim
  ...
  if (!legacyAppWithShim) {
    ...
    if (sereneShimContent) {
      scripts.push(`<script>${sereneShimContent}</script>`);
    } else {
      scripts.push(
        '<script>console.warn("[proxy-worker] serene shim was not injected.");</script>',
      );
    }
  }
  if (sereneComponentSelectorClientContent) {
    scripts.push(`<script>${sereneComponentSelectorClientContent}</script>`);
  } else {
    scripts.push(
      '<script>console.warn("[proxy-worker] serene component selector client was not injected.");</script>',
    );
  }
