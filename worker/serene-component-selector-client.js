(() => {
  const OVERLAY_ID = "__serene_overlay__";
  let overlay, label;
  ...
  function updateOverlay(el, isSelected = false) {
    ...
    const name = el.dataset.sereneName || "<unknown>";
    const file = (el.dataset.sereneId || "").split(":")[0];
  ...
  function onMouseMove(e) {
    ...
    let el = e.target;
    while (el && !el.dataset.sereneId) el = el.parentElement;
  ...
  function onClick(e) {
    ...
    window.parent.postMessage(
      {
        type: "serene-component-selected",
        id: state.element.dataset.sereneId,
        name: state.element.dataset.sereneName,
      },
      "*",
    );
  }
  ...
  window.addEventListener("message", (e) => {
    ...
    if (e.data.type === "activate-serene-component-selector") activate();
    if (e.data.type === "deactivate-serene-component-selector") deactivate();
  });

  function initializeComponentSelector() {
    if (!document.body) {
      console.error(
        "Serene component selector initialization failed: document.body not found.",
      );
      return;
    }
    setTimeout(() => {
      if (document.body.querySelector("[data-serene-id]")) {
        window.parent.postMessage(
          {
            type: "serene-component-selector-initialized",
          },
          "*",
        );
        console.debug("Serene component selector initialized");
      } else {
        console.warn(
          "Serene component selector not initialized because no DOM elements were tagged",
        );
      }
    }, 0);
  }
